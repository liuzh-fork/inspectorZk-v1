...
public class GetZNodeController {
    ...
    @FXML
    private CheckBox deleteAllChildren;
    ...
    private void deleteChildButtonAction() {
        String basePath = pathTextField.getText();
        String selectedPath = childListView.getSelectionModel().getSelectedItem();
        String fullPath = basePath + (basePath.equals("/") ? "" : "/") + selectedPath;
        ...
        if (yes) {
            ...
            try {
                if (deleteAllChildren.isSelected()) {
                    deleted = zkService.deleteZnodeAndChildren(fullPath);
                } else {
